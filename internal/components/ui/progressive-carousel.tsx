...
export const ProgressSlider: FC<ProgressSliderProps> = ({
  ...
  useEffect(() => {
    ...
    if (getChildren && React.isValidElement(getChildren)) {
      const childrenProps = getChildren.props as { children: ReactNode };
      const values = React.Children.toArray(childrenProps.children).map(
        (child) => {
          if (React.isValidElement(child)) {
            return (child.props as { value?: string }).value as string;
          }
          return '';
        }
      ).filter(Boolean);
...
export const SliderBtn: FC<SliderBtnProps> = ({
  ...
  return (
    <button
      className={cn(
        `relative group overflow-hidden rounded-xl border border-border/50 bg-card/50 text-left transition-all duration-300 hover:bg-muted/50 focus-ring ${
          active === value
            ? 'opacity-100 ring-1 ring-primary/20 bg-muted/30'
    ...
    >
      ...
      >
        <span
          className={cn('absolute bottom-0 left-0 bg-primary/10 transition-all duration-100 ease-linear', progressBarClass)}
          style={{
